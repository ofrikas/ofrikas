<div align="center">

  <h1>Ofri Kastenbaum</h1>
  <h3>Cybersecurity Architect | AI Agent Developer</h3>

  <p>
    <br>
    I operate at the intersection of Cybersecurity and Autonomous AI.
    <br>
    My focus is building <b>agentic workflows</b> (using tools like <b>LangChain</b>) that don't just collect data—they understand it. I build systems that autonomously parse complex security logs to identify relevant threats that traditional automation misses.
  </p>
  
  <br>

  <a href="https://linkedin.com/in/ofri-kastenbaum">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
  <a href="mailto:ofrikas1@gmail.com">
    <img src="https://img.shields.io/badge/Email-Contact_Me-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
  </a>

</div>

<br>

<div align="center">
  <img src="./profile-banner.jpg" alt="Ofri working on Cyber Automation" width="100%" style="border-radius: 10px; border: 2px solid #30363d;" />
</div>

<br>
<hr>

### 💬 Ask Me About
* **Agentic Workflows:** How to move from simple scripts to autonomous agents that can reason and act.
* **LangChain in Security:** Using LLMs to "read" vulnerabilities and map them to real-world relevance.
* **Scalable Automation:** Turning manual security checks into 24/7 autonomous loops.

<br>

### 🛠 Tech Stack

<div>
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" alt="Python" />
  <img src="https://img.shields.io/badge/LangChain-E1382C?style=for-the-badge&logo=chainlink&logoColor=white" alt="LangChain" />
  <img src="https://img.shields.io/badge/OpenAI_API-412991?style=for-the-badge&logo=openai&logoColor=white" alt="OpenAI" />
</div>

<div style="margin-top: 5px">
  <img src="https://img.shields.io/badge/Cybersecurity-FF0000?style=for-the-badge&logo=security&logoColor=white" alt="Cybersecurity" />
  <img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white" alt="Docker" />
  <img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black" alt="Linux" />
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white" alt="Git" />
</div>

<br>
<hr>

### 📊 GitHub Activity

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=ofrikas&show_icons=true&theme=radical&hide_border=true&count_private=true&include_all_commits=true" alt="Ofri's Stats" />
</div>
